...
(define-public (burn-token-supply (some-value uint))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (try! (contract-call? .token-test mint some-value tx-sender))
    (try! (contract-call? .token-test burn some-value tx-sender))
    (ok true)))
...
(define-private (public-mint (new-owner principal))
  (begin
    ...
    (try! (contract-call? .phase1-test-t0 mint new-owner))
    (ok true)))

;; Register this contract as allowed to mint
(as-contract (contract-call? .phase1-test-t0 set-mint-address))
(as-contract (contract-call? .phase2-test-t0 set-mint-address))
(as-contract (contract-call? .token-test set-mint-address))