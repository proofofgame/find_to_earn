...
;; Define NFT token
(define-non-fungible-token skullcoin_competitive_bb_p1 uint)
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? skullcoin_competitive_bb_p1 id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? skullcoin_competitive_bb_p1 next-id new-owner)
        success
        (let
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? skullcoin_competitive_bb_p1 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? skullcoin_competitive_bb_p1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? skullcoin_competitive_bb_p1 id) ERR-NOT-FOUND))
