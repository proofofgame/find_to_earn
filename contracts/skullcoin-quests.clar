...
(define-read-only (get-quest-players (quest-id uint))
    (unwrap! (get players-list (map-get? players { quest-id: quest-id}))))
...
(define-public (initialize-quest (quest-id uint) (price uint) (slots uint))
    (begin
        (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
        (var-set sale-active true)
        (var-set current-quest quest-id)
        (var-set amount price)
...
(define-public (sign-up (memo (string-ascii 100)))
    (begin
        (asserts! (var-get sale-enabled) ERR-SALE-NOT-ACTIVE)
        (asserts! (not (is-eq (var-get remaining-slots) u0)) ERR-OUT-OF-SLOTS)
        (asserts! (is-eq (get data (unwrap-panic (map-get? data (tuple (quest-id (var-get current-quest)) (player tx-sender)))) true)) ERR-ALREADY-IN-QUEST)
        ...
        (map-set players { quest-id: (var-get current-quest) } { players-list: (concat (unwrap! (get players-list (map-get? players { quest-id: (var-get current-quest)})))  (concat memo ";")) } )
